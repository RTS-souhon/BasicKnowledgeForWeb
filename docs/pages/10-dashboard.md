...
│  ロール:  スタッフ                     │  ← user → "スタッフ"
│                                      │     admin → "管理者"
│  ── パスワード変更 ──                  │     developer → "開発者"
│  現在のパスワード [          ]         │
│  新しいパスワード [          ]         │
│  確認            [          ]         │
│  [変更する]                           │
│                                      │
│  ── ユーザー管理（admin/developer のみ）──  │
│  名前          メール               ロール    操作  │
│  ─────────────────────────────────────────       │
...
```json
{ "users": [ { "id": "...", "name": "...", "email": "...", "role": "user" | "admin" | "developer" } ] }
```

### PUT `/api/users/:id/role` （admin/developer）
...
- `page.tsx`: Server Component で `/api/auth/me` と `/api/users`（admin/developer の場合のみ）を fetch
- パスワード変更フォーム: Client Component
  - `react-hook-form` + `changePasswordSchema`
  - 成功時: 成功メッセージ表示 + フォームリセット
- admin/developer 判定: Server Component 側でロール確認 → ユーザー管理・管理メニューの表示切り替え
- ロール変更 UI: Client Component
  - 各ユーザー行に [変更] ボタン → ドロップダウンで `user` / `admin` / `developer` を選択
...
| 7 | ユーザー一覧が表示されること（admin/developer） |
